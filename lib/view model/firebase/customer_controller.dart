...
class CustomerController extends GetxController{
  ...
  editCustomerData(String customerId)async{
    var customerModel = {
      "customerName": customerName.value.text,
      "email": customerEmail.value.text,
      "phoneNumber": customerPhone.value.text,
    ...
    setLoading(true);
    try{
      await FirebaseFirestore.instance.collection("users").doc(FirebaseAuth.instance.currentUser!.uid).collection("customer").doc(customerId).set(customerModel).then((value){
        setLoading(false);
        Get.back();
        Get.back();
      }).onError((error, stackTrace){
